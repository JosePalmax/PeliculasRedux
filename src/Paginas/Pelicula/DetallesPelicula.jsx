...
import { setFavoritas, deleteFavoritas } from "../../slices/peliSlice";
...
const Pelicula = () => {
    ...
    let favs = useSelector(state => state.pelis.favoritas)
    ...
    const marcarFavorita = (e) => {
        if (e.currentTarget.style.color != 'red') {
            dispatch(setFavoritas({ 'id': id, 'foto': `https://image.tmdb.org/t/p/w500${peli.poster_path}` }))
            e.currentTarget.style.color = 'red'
        } else if (e.currentTarget.style.color == 'red') {
            dispatch(deleteFavoritas(id))
